...
export function useFileSystem() {
  ...
  const navigateTo = useCallback(async (tabId: string, dirPath: string) => {
    setTabs(prev => prev.map(tab => {
      ...
      const newHistory = [...tab.history.slice(0, tab.historyIndex + 1), dirPath];
      const parts = dirPath.split('/').filter(Boolean);
      return {
        ...tab,
        path: dirPath,
  ...
  const pasteFiles = useCallback(async () => {
    ...
    if (clipboard.operation === 'copy') {
      await window.api.copyItems(clipboard.paths, tab.path);
    } else {
      await window.api.moveItems(clipboard.paths, tab.path);
      setClipboard(null);
    }
  ...
  const deleteFiles = useCallback(async () => {
    ...
    await window.api.deleteItems(Array.from(selectedFiles));
    setSelectedFiles(new Set());
  ...
  return {
    ...
    renameFile, createNewFolder, refreshCurrentDir,
    search, getSortedFiles,
  };
}
